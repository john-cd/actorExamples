...
/** Wrapper around WatchService - monitor changes to the file system
  ...
  */
object WatchDir {
  def apply(created: (Path, Path) => Unit, modified: (Path, Path) => Unit, deleted: (Path, Path) => Unit): WatchDir = new WatchDir(created, modified, deleted)
}

@throws[IOException]
class WatchDir(created: (Path, Path) => Unit, modified: (Path, Path) => Unit, deleted: (Path, Path) => Unit)
  extends LazyLogging {
  ...
  def register(start: Path, recursive: Boolean = false): Unit = {
    if (recursive) {
      //logger.info(s"WatchDir - register $start ...")
      registerAll(start, trace = false)
      //logger.info("WatchDir - registered.")
    }
  ...
  private def registerOne(dir: Path, trace: Boolean = true): Unit = {
    ...
    if (trace)
      keys.get(key) match {
        ...
        case Some(prev) => logger.info(s"Registration $prev")
        ...
        key.pollEvents().asScala.foreach(e => {
          ...
          event.kind() match {
            ...
            case StandardWatchEventKinds.ENTRY_CREATE =>
              //logger.info(s"WatchDir - Entry created: $path")
              // TODO provide option to register or not, recursively or not
              if (Files.isDirectory(path)) {
                registerAll(path)
              }
              created(registeredPath, path) //  example: notifyActor ! Created(path)
            ...
            case StandardWatchEventKinds.ENTRY_DELETE => // Directory entry deleted.
              //logger.info(s"WatchDir - Entry deleted: $path")
              deleted(registeredPath, path) // example: notifyActor ! Deleted(path)

            // an entry in the registered directory has been modified.
            case StandardWatchEventKinds.ENTRY_MODIFY =>
              //logger.info(s"WatchDir - Entry modified: $path")
              modified(registeredPath, path)
